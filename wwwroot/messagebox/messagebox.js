...
MessageBox - A jQuery Plugin to replace Javascript's window.alert(), window.confirm() and window.prompt() functions
    ...
    Modified by Dr Andy's IP to add markdown and DARL editiing.
***********************************************************************************************************************/
(function ($, undefined) {
    "use strict";
    ...
    function _CreateMessageBox(instance) {
        ...
        if (settings.input !== false && settings.input !== undefined && settings.input !== null) {
            ...
            //add editors here
            inputs.find("textarea").each(function (index, element) {
                switch (element.getAttribute("editor")) {
                    case "code":
                        CreateCodeEditor(element, element.value);
                        break;
                    case "markdown":
                        CreateMarkdownEditor(element, element.value);
                        break;
                }
            });
            ...
            if (input.attr("editor")) {
                switch (input.attr("editor")) {
                    case "code":
                        ...
                        break;
                    case "markdown":
                        var markdown = editors[0].codemirror.getValue();
    ...
    function CreateCodeEditor(tag, val) {
        var darlCodeMirror = CodeMirror.fromTextArea(tag, {
            matchBrackets: true,
        ...
        editors.push(darlCodeMirror);
    }

    function CreateMarkdownEditor(tag, val) {
        var mde = new SimpleMDE({ element: tag });
        mde.value(val);
        editors.push(mde);
    }
    ...
    function _Window_KeyDown(event) {
        ...
        if (button) {
            var messagebox = button.closest(".messagebox");
            if (event.which === 13 && messagebox.find(".messagebox_content_input_textarea:focus, .CodeMirror").length) return;   // Allows newline in textarea
